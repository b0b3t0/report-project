...
import com.mentormate.report.util.FileCreator;
...
public class ReportConsoleApplication {
    public static void main(String[] args) {
        ...
        SellerDataDto[] data1 = runCommand("read json data", () ->
               FileUtils.readJson(dataFilePath, SellerDataDto[].class));

        ReportRulesDto data2 = runCommand("read jason rules", () ->
                 FileUtils.readJson(reportDefinitionFilePath, ReportRulesDto.class));

        runCommand("Write csv file", () -> {
            File file = FileCreator.createFile(outputFilePath);
            List<SellerResult> results = ReportResultCalculator.calculateResult(data1, data2);
